...
const UpcomingQuests = ({ tasks, category }) => {
    ...
    <div className="todays-tasks">
      <div className="todays-task-icon-title">
        ...
        <div className="todays-task-title">
          ...
          <p>
            {uncompletedTasks.length > 0 && "Quests queued: Awaiting dispatch"}
          </p>
        </div>
      </div>
      ...
      <div className="todays-task-wrapper">
        {uncompletedTasks.length > 0 ? (
          uncompletedTasks.map((item,idx) => (
            <div key={idx} className="todays-task-cards">
              <h4>{item.category}</h4>
              <h3>{item.title}</h3>
              <p>
          ...
          <div className="empty-state-img">
            ...
            <p className="empty-title">You're all clear!</p>
            <p className="empty-title">Add a new task to begin.</p>
          </div>
        )}
      </div>
    </div>
